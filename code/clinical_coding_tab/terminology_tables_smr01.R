...
RCodes <- last_episode2 %>% 
  ...
  mutate(
    resp_chest = case_when(
      str_detect(main_condition, c("^R05", "^R06", "^R07")) ~ "resp/chest",
      TRUE ~ 'other')) %>% 
  mutate(
    APV = case_when(
...
#summarise counts by clinical code groupings
RCodes2 <- last_episode2 %>% 
  group_by(HBName, year) %>% 
...
write_csv(RCodes_multi, "/conf/Data_Quality_Dashboard/data/RCodes.csv")
